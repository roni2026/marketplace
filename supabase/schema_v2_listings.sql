...
do $$ begin create type public.condition_grade as enum ('new', 'like_new', 'good', 'fair', 'poor'); exception when duplicate_object then null; end $$;
...
-- -------------------------------------------------------------------------
drop trigger if exists trg_listing_variants_updated_at on public.listing_variants;
create trigger trg_listing_variants_updated_at
  before update on public.listing_variants
  for each row execute procedure public.set_updated_at();

drop trigger if exists trg_listing_templates_updated_at on public.listing_templates;
